...
function CodeActionPanel:build_content(actions, diag_code)
  ...
  for i, entry in ipairs(actions) do
    ...
    table.insert(ext_list, { sign = { icon = icon, hl = "GloseActionNumber" }, line_hl = "Normal", text_hl = "Normal" })
  end
...
local function apply_action_at(idx)
  ...
  if not client then
    vim.notify("Glose: Client not found", vim.log.levels.ERROR)
    return
  end
...
function M.code_action()
  ...
  if #clients == 0 then
    vim.notify("Glose: No LSP clients support code actions", vim.log.levels.INFO)
    return
  end
      ...
      if pending == 0 then
        vim.schedule(function()
          if #all_actions == 0 then
            vim.notify("Glose: No code actions available", vim.log.levels.INFO)
            return
          end
