...
local extmarks_ui = require "glose.ui.extmarks"
local footer = require "glose.ui.footer"
local FloatPanel = require "glose.ui.float"
local PreviewManager = require "glose.ui.preview"
...
local function execute_action(idx)
  ...
  if not entry then
    if #preview.action_cache == 0 then
      vim.notify("Glose: No code actions available yet", vim.log.levels.INFO)
    end
    return
  end
  ...
  if not client then
    vim.notify("Glose: Client not found for action", vim.log.levels.ERROR)
    return
  end
...
local function request_code_actions()
  ...
  local kinds = require("glose.config").get().diagnostic.code_action_kinds
      ...
      if pending == 0 then
        vim.schedule(function()
          ...
          for i, entry in ipairs(all_actions) do
            ...
            table.insert(
              action_extmarks,
              { sign = { icon = icon, hl = "GloseActionNumber" }, line_hl = "Normal", text_hl = "Normal" }
            )
          end
          ...
          extmarks_ui.apply(target_buf, "glose_diagnostic", action_extmarks, action_lines, #current_lines)
