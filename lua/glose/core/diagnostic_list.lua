...
---@class Glose.DiagnosticListPanel : Glose.FloatPanel
...
local panel = DiagnosticListPanel:new "diagnostic_list" --[[@as Glose.DiagnosticListPanel]]
...
function DiagnosticListPanel:open_win(lines)
  ...
  vim.api.nvim_set_option_value(
    "winhighlight",
    "Normal:GloseNormal,SignColumn:GloseNormal,CursorLine:CursorLine",
    { win = self.win }
  )
...
function DiagnosticListPanel:build_content(scope, prefetched)
  ...
  if #diagnostics == 0 then
    vim.notify("Glose: No diagnostics in " .. (scope == "workspace" and "workspace" or "buffer"), vim.log.levels.INFO)
    return {}, {}
  end
  ...
  if scope == "workspace" then
    ...
    for _, bufnr in ipairs(order) do
      ...
      local file_icon, file_hl = glyph.ui.file, "GloseDiffFile"
      ...
      lines[#lines + 1] = short
      ext_list[#ext_list + 1] = { sign = { icon = file_icon, hl = file_hl }, line_hl = "GloseDiffFile" }
    ...
    for _, k in ipairs(items) do
      if cfg.footer.show_desc then
        parts[#parts + 1] = string.format(
          "%%#GloseFooterIcon# %s%%#GloseFooterDesc# %s%%#GloseFooterKey#[%s] ",
          k.icon,
          k.desc,
          display_key(k.key)
        )
      else
        parts[#parts + 1] = string.format("%%#GloseFooterIcon# %s%%#GloseFooterKey#[%s] ", k.icon, display_key(k.key))
      end
    end
...
function M.open(scope, source_bufnr)
  ...
  scope = scope or require("glose.config").get().diagnostic_list.scope or "buffer"
