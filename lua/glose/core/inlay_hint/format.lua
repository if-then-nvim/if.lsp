...
function M.build_field_chunks(field, border, padded_name, obj_hl)
  ...
  local field_hl = type_kind and ("GloseInlayTypeObjField_" .. type_kind) or obj_hl
  ...
  if type_kind == "function" then
    ...
    if ret then
      ...
      local ret_hl = ret_kind and ("GloseInlayTypeObjField_" .. ret_kind) or field_hl
      ...
      table.insert(chunks, { arrow .. " ", "GloseInlayTypeObjOperator" })
...
function M.build_generic_param_chunks(param_text, border, base_hl, hide_icon)
  ...
  local param_hl = type_kind and ("GloseInlayTypeObjField_" .. type_kind) or base_hl
...
function M.build_function_chunks(label, icon, hl, cfg)
  ...
  local ret_icon, ret_kind = detect_type(ret)
  local ret_hl = ret_kind and ("GloseInlayTypeFnRet_" .. ret_kind) or hl
  ...
  local result = {
    chunks = {
      { " ", "" },
      { fn_part, hl },
      { (glyph.inlay.arrow or "→") .. " ", "GloseInlayTypeOperator" },
      { ret_part, ret_hl },
    },
  }
...
function M.format_hint(hint, cfg)
  ...
  if is_param then
    ...
    hl = type_kind and ("GloseInlayType_" .. type_kind) or "GloseInlayType"
    if type_kind == "function" then
