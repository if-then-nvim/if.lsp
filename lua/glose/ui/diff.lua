...
function M.build_lines(diffs)
  ...
  for _, file_diff in ipairs(diffs) do
    ...
    table.insert(
      extmarks,
      { sign = { icon = file_icon, hl = file_hl }, line_hl = "GloseDiffFile", text_hl = "GloseDiffFile" }
    )

    for diff_line in file_diff.diff:gmatch "[^\n]+" do
      if not (diff_line:match "^%-%-%-" or diff_line:match "^%+%+%+") then
        if diff_line:match "^@@" then
          ...
          table.insert(extmarks, {
            sign = { icon = glyph.diff.hunk_sign, hl = "GloseDiffHunk" },
            line_hl = "GloseDiffHunk",
            text_hl = "GloseDiffHunk",
          })
        elseif diff_line:match "^%+" then
          table.insert(lines, diff_line:sub(2))
          table.insert(
            extmarks,
            { sign = { icon = glyph.diff.add, hl = "GloseDiffAddSign" }, line_hl = "GloseDiffAdd" }
          )
        ...
        elseif diff_line:match "^%-" then
          ...
          table.insert(
            extmarks,
            { sign = { icon = glyph.diff.delete, hl = "GloseDiffDeleteSign" }, line_hl = "GloseDiffDelete" }
          )
...
function M.apply_syntax(buf, code_info, ft, buf_offset)
  local syntax_ns = vim.api.nvim_create_namespace "glose_diff_syntax"
