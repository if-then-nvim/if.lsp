local extmarks = require "glose.ui.extmarks"
local meta_ui = require "glose.ui.meta"
local scrollbar = require "glose.ui.scrollbar"
...
function FloatPanel:close()
  ...
  if buf and vim.api.nvim_buf_is_valid(buf) then
    vim.api.nvim_buf_clear_namespace(buf, vim.api.nvim_create_namespace "glose_diff_syntax", 0, -1)
  end
...
function FloatPanel:apply_extmarks(ext_list, lines)
  extmarks.apply(self.buf, "glose_" .. self.name, ext_list, lines, 0)
end
...
function FloatPanel:setup_autocmds()
  local cfg = self:get_config()
  local panel = self
  local augroup = vim.api.nvim_create_augroup("glose_" .. self.name .. "_close", { clear = true })
...
function FloatPanel:get_config()
  return require("glose.config").get()[self.name] or {}
end
