...
function M.parse_object_fields(label)
  ...
  for i = 1, #inner do
    ...
    if ch == "(" or ch == "{" or ch == "<" then
      depth = depth + 1
      current = current .. ch
    elseif ch == ")" or ch == "}" or (ch == ">" and current:sub(-1) ~= "=") then
      depth = depth - 1
      current = current .. ch
...
function M.build_field_chunks(field, border, padded_name, obj_hl)
  ...
  local field_hl = type_kind and ("IfLspInlayTypeObjField_" .. type_kind) or obj_hl
  ...
  if type_kind == "function" then
    ...
    if ret then
      ...
      local ret_hl = ret_kind and ("IfLspInlayTypeObjField_" .. ret_kind) or field_hl
      ...
      table.insert(chunks, { arrow .. " ", "IfLspInlayTypeObjOperator" })
...
function M.build_generic_param_chunks(param_text, border, base_hl, hide_icon)
  ...
  local param_hl = type_kind and ("IfLspInlayTypeObjField_" .. type_kind) or base_hl
...
function M.build_function_chunks(label, icon, hl, cfg)
  ...
  local ret_icon, ret_kind = detect_type(ret)
  local ret_hl = ret_kind and ("IfLspInlayTypeFnRet_" .. ret_kind) or hl
  ...
  local result = {
    chunks = {
      { " ", "" },
      { fn_part, hl },
      { (glyph.inlay.arrow or "→") .. " ", "IfLspInlayTypeOperator" },
      { ret_part, ret_hl },
    },
  }
...
function M.format_hint(hint, cfg)
  ...
  local fmt = is_param and cfg.param_format or cfg.type_format
  ...
  if is_param then
    ...
    hl = type_kind and ("IfLspInlayType_" .. type_kind) or "IfLspInlayType"
    if type_kind == "function" then
