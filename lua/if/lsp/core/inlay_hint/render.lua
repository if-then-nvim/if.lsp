...
local format = require "if.lsp.core.inlay_hint.format"
local iconify = require "if.lsp.core.inlay_hint.iconify"

local function get_cfg()
  return require("if.lsp.config").get().inlay_hint
end
...
function M.render(bufnr, state, ns)
  ...
  for _, entry in ipairs(render_entries) do
    ...
    if #chunks > 0 and pos.lnum < vim.api.nvim_buf_line_count(bufnr) then
  ...
  for _, entry in ipairs(render_entries) do
    ...
    if expand_data then
      ...
      local function render_expand_list(prefix_chunks, item_chunk_list)
        ...
        local header_col = expand_line_offset == 0 and win_col or item_col

        if cfg.expand_border and #render_lines > 1 then
          local max_cw = 0
          for _, rline in ipairs(render_lines) do
            local cw = 0
            for ci = 2, #rline do
              cw = cw + vim.fn.strdisplaywidth(rline[ci][1])
            end
            if cw > max_cw then
              max_cw = cw
            end
          ...
          local box_hl = render_lines[1][1][2]
          local inner_w = max_cw + 2
          ...
          for _, rline in ipairs(render_lines) do
            ...
            local hl = rline[1][2]
            rline[1] = { "│ ", hl }
            table.insert(rline, { string.rep(" ", gap + 1) .. "│", hl })
          end

          local top_lnum = header_lnum - 1
          if top_lnum >= 0 and top_lnum < line_count then
            vim.api.nvim_buf_set_extmark(bufnr, ns, top_lnum, 0, {
              virt_text = { { "╭" .. string.rep("─", inner_w) .. "╮", box_hl } },
          ...
          local bot_lnum = pos.lnum + expand_line_offset + #render_lines
          if bot_lnum < line_count then
            vim.api.nvim_buf_set_extmark(bufnr, ns, bot_lnum, 0, {
              virt_text = { { "╰" .. string.rep("─", inner_w) .. "╯", box_hl } },
              virt_text_pos = "overlay",
              virt_text_win_col = item_col,
              priority = 2047,
        ...
        local field_start = #render_lines == 1 and 2 or 1
        for ci = field_start, #render_lines[1] do
          table.insert(header_chunks, render_lines[1][ci])
        end
      ...
      elseif expand_data.params then
        ...
        if #params == 1 then
          local win_col = calc_win_col()
          local single = format.build_generic_param_chunks(params[1], "", base_hl, hide_icon)
          ...
          for ci = 2, #single do
            table.insert(inline_chunks, single[ci])
          end
          table.insert(inline_chunks, { " ›", base_hl })
          local lnum = pos.lnum + expand_line_offset
