...
function M.build_lines(diffs)
  ...
  for _, file_diff in ipairs(diffs) do
    ...
    table.insert(
      extmarks,
      { sign = { icon = file_icon, hl = file_hl }, line_hl = "IfLspDiffFile", text_hl = "IfLspDiffFile" }
    )
      ...
      if not (diff_line:match "^%-%-%-" or diff_line:match "^%+%+%+") then
        if diff_line:match "^@@" then
          ...
          table.insert(extmarks, {
            sign = { icon = glyph.diff.hunk_sign, hl = "IfLspDiffHunk" },
            line_hl = "IfLspDiffHunk",
            text_hl = "IfLspDiffHunk",
          })
        elseif diff_line:match "^%+" then
          table.insert(lines, diff_line:sub(2))
          table.insert(
            extmarks,
            { sign = { icon = glyph.diff.add, hl = "IfLspDiffAddSign" }, line_hl = "IfLspDiffAdd" }
          )
        ...
        elseif diff_line:match "^%-" then
          ...
          table.insert(
            extmarks,
            { sign = { icon = glyph.diff.delete, hl = "IfLspDiffDeleteSign" }, line_hl = "IfLspDiffDelete" }
          )
...
function M.apply_syntax(buf, code_info, ft, buf_offset)
  local syntax_ns = vim.api.nvim_create_namespace "iflsp_diff_syntax"
