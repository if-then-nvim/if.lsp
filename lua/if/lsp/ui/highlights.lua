...
local function create_scope_highlights()
  ...
  if not user_overrides["IfLspScopeBiscuitKeyword"] then
    vim.api.nvim_set_hl(0, "IfLspScopeBiscuitKeyword", biscuit_opts(kw_fg))
  end

  if not user_overrides["IfLspScopeText"] then
    vim.api.nvim_set_hl(0, "IfLspScopeText", { link = "Normal" })
  end
  if not user_overrides["IfLspScopeSeparator"] then
    vim.api.nvim_set_hl(0, "IfLspScopeSeparator", { link = "Comment" })
  end
end
...
local function create_badge_highlights()
  ...
  local badge_groups = {
    IfLspInlayType = { fg = blended_blue_fg, bg = blended_blue_bg, italic = true },
    IfLspInlayParam = { fg = blended_warm_fg, bg = blended_warm_bg, italic = true },
  }

  for kind, sources in pairs(type_hl_sources) do
    local src_fg = resolve_hl_fg(sources, blue_tint)
    local name = "IfLspInlayType_" .. kind
    badge_groups[name] = {
  ...
  local fn_bg = badge_groups["IfLspInlayType_function"].bg
  local operator_fg = resolve_hl_fg({ "@operator.tsx", "@operator" }, comment_fg)
  badge_groups["IfLspInlayTypeOperator"] = {
  ...
  for kind, sources in pairs(type_hl_sources) do
    if kind ~= "function" then
      local src_fg = resolve_hl_fg(sources, blue_tint)
      badge_groups["IfLspInlayTypeFnRet_" .. kind] = {
        fg = color.blend(src_fg, comment_fg, 0.4),
        bg = fn_bg,
        italic = true,
      }
  ...
  local obj_bg = badge_groups["IfLspInlayType_object"].bg
  for kind, sources in pairs(type_hl_sources) do
    local src_fg = resolve_hl_fg(sources, blue_tint)
    badge_groups["IfLspInlayTypeObjField_" .. kind] = {
      fg = color.blend(src_fg, comment_fg, 0.4),
      bg = obj_bg,
      italic = true,
    }
  ...
  badge_groups["IfLspInlayTypeObjOperator"] = {
...
function M.setup()
  local highlights = {
    IfLspNormal = { link = "NormalFloat" },
    IfLspBorder = { link = "FloatBorder" },
    IfLspTitle = { link = "Title" },
    IfLspHoverKind = { link = "Function" },
    IfLspHoverKindAlias = { link = "Special" },
    IfLspHoverKindFunction = { link = "Function" },
    IfLspHoverKindProperty = { link = "@property" },
    IfLspHoverKindVariable = { link = "@variable" },
    IfLspHoverKindType = { link = "Type" },
    IfLspHoverKindEnum = { link = "Constant" },
    ...
    IfLspDiffDeleteSign = { link = "Removed" },
    IfLspDiffHunk = { link = "Comment" },
    IfLspDiffFile = { link = "Normal" },
    IfLspFooterIcon = { link = "Function" },
    IfLspFooterDesc = { link = "Comment" },
    IfLspFooterKey = { link = "Function" },
    IfLspSignatureActiveParam = { link = "LspSignatureActiveParameter" },
  }
  ...
  vim.api.nvim_create_autocmd("ColorScheme", {
    group = vim.api.nvim_create_augroup("iflsp_badge_colors", { clear = true }),
    callback = function()
      create_badge_highlights()
      create_scope_highlights()
    end,
