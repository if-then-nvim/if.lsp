...
local function build_server(meta, server_name)
  if server_name then
    table.insert(meta, { icon = glyph.ui.server, text = server_name, hl = "IfLspTitle", line_hl = "IfLspTitle" })
  end
end
...
local function build_filetype(meta, source_bufnr)
  ...
  table.insert(meta, { icon = ft_icon, text = ft, hl = ft_hl or "IfLspTitle" })
end
...
local function build_symbol(meta, source_bufnr)
  ...
  table.insert(meta, { icon = sym_icon, text = symbol, hl = sym_capture or "IfLspTitle", text_hl = sym_capture })
end
