...
describe("glose.ui.color", function()
  ...
  describe("get_hl_color", function()
    it("returns hex string for set highlight", function()
      vim.api.nvim_set_hl(0, "GloseColorTestA", { fg = "#abcdef" })
      assert.equal("#abcdef", color.get_hl_color("GloseColorTestA", "fg"))
    end)

    it("returns nil when attribute missing", function()
      vim.api.nvim_set_hl(0, "GloseColorTestEmpty", {})
      assert.is_nil(color.get_hl_color("GloseColorTestEmpty", "fg"))
    end)

    it("returns hex string for bg attr", function()
      vim.api.nvim_set_hl(0, "GloseColorTestB", { bg = "#123456" })
      assert.equal("#123456", color.get_hl_color("GloseColorTestB", "bg"))
    end)
  end)
end)
