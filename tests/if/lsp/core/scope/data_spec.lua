...
describe("if.lsp.core.scope.data", function()
  ...
  describe("format_path", function()
    ...
    it("places separator between items", function()
      ...
      assert.equal("IfLspScopeSeparator", chunks[2][2])
    ...
    it("prepends indicator when depth exceeds limit", function()
      ...
      assert.equal("IfLspScopeSeparator", chunks[1][2])
    end)

    it("uses IfLspScopeKind_<kind> hl for symbol chunks", function()
      local path = { { name = "fn", kind = 10 } }
      local chunks = data.format_path(path, " > ", 0, "...")
      assert.equal("IfLspScopeKind_10", chunks[1][2])
    end)
  end)
